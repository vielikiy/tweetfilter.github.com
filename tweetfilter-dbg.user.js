...
//    this.enableoption(['show-unfollowers'], this._page === 'Home' && !!document.getElementById('tf-unfollowers-component'));
    ...
    if (arguments.length > 0) {
      ...
      for (var e=0,emax=event.length;e<emax;e++) {
        if (this.polling.events.hasOwnProperty(event[e])) {
                                                                                                    _D('F:poll', 'queueing', event[e], ' for tick',this.polling.tick+1,'with params:', params);
          switch(typeof params) {
            ...
            default:
                                                                                                    _D('F:poll', 'queued event ', event[e], ', params:', this.polling.queued[event[e]]);
      ...
      for (var e in this.polling.events) {
        if (this.polling.suspend) {
          ...
          this.widget.removeClass('busy');
                                                                                                    _D('F:_poll', 'W:polling suspended by trigger!', e);
          return;        
        }
        if (this._isloading) {
                                                                                                    _D('F:_poll', 'W:interrupted, is loading!', e);
          return;
        }
    ...
    if (this.polling.timeoutid === -1) {
      this.polling.timeoutid = setTimeout((function () {
        ...
        this._poll();
                                                                                                    ...
                                                                                                    _D('F:_breathe', 'D:NOT repolling tick '+this.polling.tick+', already queued!');
    }
  };
  ...
  Tweetfilter.prototype.lockscroll = function() {
    if (this.getoption('scroll-lock')) {
      ...
      if (this.stream.isready()) {
                                                                                                    _D(f, 'stream is ready, new items loaded:', this.stream.newitemsloaded);
        if (!this.status.scrollsinceid) {
                                                                                                    _D(f, 'W:scrollsinceid not set, initializing scroll status');
          this.status.scrollsinceid = this.stream.sinceid;
          this.updatescrollstatus();
          return true;
        }
        if (!this.status.scrollheight) {
                                                                                                    _D(f, 'W:scrollheight not set, initializing scroll status');
          this.updatescrollstatus();
          return true;
        }
        if (this.stream.newitemsloaded) {
          if (this.stream.sinceid !== this.status.scrollsinceid || twttr.$doc.height() !== this.status.scrollheight) {
            var docheight = twttr.$doc.height();
                                                                                                    _D(f, 'sinceid changed?', this.stream.sinceid, '!=', this.status.scrollsinceid);
                                                                                                    _D(f, 'docheight changed?', docheight, '!=', this.status.scrollheight, ':',  docheight - this.status.scrollheight);
            this.status.scrolltop += docheight - this.status.scrollheight;
                                                                                                    _D(f, 'W:scrolling to:', this.status.scrolltop);
  ...
  Tweetfilter.prototype.loadsettings = function(imported) {
    ...
    if (typeof imported === 'undefined') {
      ...
      this.status.mentionslastchecked = settings.hasOwnProperty('mentionslastchecked') ? settings.mentionslastchecked : -1;
      this.status.messageslastchecked = settings.hasOwnProperty('messageslastchecked') ? settings.messageslastchecked : -1;
  ...
  Tweetfilter.prototype.savesettings = function(imported) {   
    ...
    if (typeof imported != 'undefined') {
      ...
      settings[this.user.id].messageslastchecked = this.status.messageslastchecked > 0 ? this.status.messageslastchecked : -1;
      settings[this.user.id].mentionsinceid = this.status.mentionsinceid > 0 ? this.status.mentionsinceid : -1;
      settings[this.user.id].mentionslastchecked = this.status.mentionslastchecked > 0 ? this.status.mentionslastchecked : -1;
    } else {
      var mentionslastchecked = this.getstoredvalue('mentionslastchecked', -1),
          messageslastchecked = this.getstoredvalue('messageslastchecked', -1);
      if (mentionslastchecked > this.status.mentionslastchecked) {
        this.status.mentionslastchecked = mentionslastchecked;
      }
      if (messageslastchecked > this.status.messageslastchecked) {
        this.status.messageslastchecked = messageslastchecked;
      }
      settings[this.user.id] = { 
        queries: [],
        ...
        mentionslastchecked: this.status.mentionslastchecked,
        mentionsinceid: this.status.mentionsinceid > 0 ? this.status.mentionsinceid : -1,
        messageslastchecked: this.status.messageslastchecked
      };
  ...
  Tweetfilter.prototype.findcomponents = function() {
   ...
   //   hometabs.find('.stream-tab-your_activity,.stream-tab-activity').remove();
      $('.stream-tab-home', hometabs).after([
        ...
        '</li>',
        '<li class="stream-tab stream-tab-retweets dropdown-link'+(this._stream.namespace.indexOf('Retweet') > -1 ? ' active' : '')+' tf-retweets-tab">',
        ...
        '<li class="stream-tab stream-tab-activity stream-tab-your_activity dropdown-link tf-activity-tabgroup">',
          ...
          '<ul class="drop-down" style="left: 0px; right: auto; top: 30px; visibility: hidden;">',
            '<li class="stream-link" data-item-id="">',
              '<a title="Your activity" href="/#!/your_activity">',
                '<b class="item-name">',
                  'Your activity',
                '</b>',
              '</a>',
            '</li>',
            '<li class="stream-link" data-item-id="">',
              '<a title="Friend\'s activity" href="/#!/activity">',
                '<b class="item-name">',
                  'Friend\'s activity',
                '</b>',
              '</a>',
      ...
      $('.stream-tab-activity.dropdown-link a.tab-text', hometabs).bind('click', function() {
        var tab = $(this).closest('li'), submenu = $('> ul',tab);
    ...
    if (profiletabs.length && !$('.stream-tab-activity', profiletabs).length) {
      $('.stream-tab-tweets', profiletabs).after([
        '<li class="stream-tab stream-tab-activity'+(this._stream.namespace=='ActivityByUserStream' ? ' active' : '')+' tf-activity-tab tf-activity-profile-tab">',
    ...
    for (var cssclass in this.components) {
      ...
      if (container.length) {
        ...
        if (component.option) {
          if (typeof component.option === 'object') {
            enableoptions = enableoptions.concat(component.option);
          } else {
        ...
        if (typeof component.option === 'object') {
          disableoptions = disableoptions.concat(component.option);
        } else {
  ...
  Tweetfilter.prototype.setoption = function(option, status, clicked) {
    ...
    switch(option) {
      ...
      case 'hide-following':     /* hide following dashboard component */
      case 'hide-followers':      /* hide followers dashboard component */
      ...
      case 'alert-sound-message':
        ...
        if (this.status.initialized) {
          ...
          if (status) {
            _D('F:playsound', 'playing sound in setoption!');
            this.playsound();
          }
        }
      break;
  ...
  Tweetfilter.prototype.setstreamtitle = function() {
    ...
    var sm, streamtitle = this.stream.title();
                                                                                                    _D(f, 'I:setting title', streamtitle, this.stream);
    //set on widget                                                                                 
    ...
    if ((sm = this.sm)) {
     ...
     //set in stream
      var textcontainer = $('> h2', titlecontainer);
      if (!textcontainer.length) {
        textcontainer = $('<h2></h2>');
        titlecontainer.prepend(textcontainer);
      } 
      ...
      if (textcontainer.is(':empty')) {
        ...
        textcontainer = $('>span:first', textcontainer);
      }
                                                                                                    _D(f, 'textcontainer:', textcontainer, 'text:', textcontainer.text());
    ...
    switch(this.cs.streamItemType) {
      case 'activity':
        if (!this.cs.hasOwnProperty('filter')) {
          this.cs.filter = { //create filter index in cached stream
            ...
            favorited: [],  //replies and tweets beginning with @mention
            retweeted: [], //new style retweets in timeline
            listed: [],    //classic retweets in timeline
            followed: [],  //tweets containing media
            media: [],
            ...
            links: [],     //retweets or favorites containing links (* contains also media, because media is posted along with a link)
      ...
      case 'tweet': //tweet stream
        ...
        if (this.cs.filter.items.length < this.cs.items.length) {
          ...
          nextid = this.cs.filter.items.length;
          itemcount = this.cs.items.length;
          var newmentionscount = 0, mentionsinceid = this.getstoredvalue('mentionsinceid', -1);
          if (twttr.util.natcompare(mentionsinceid, this.status.mentionsinceid) > 0) {
            this.status.mentionsinceid = mentionsinceid;
          }
          while (filteredcount < itemcount) {
            if (i < filteredcount) {
          ...
          if (newmentionscount > 0) {
            ...
            if (this.options['alert-sound-mention']) {
                                                                                                    _D('F:playsound','playing sound in parsestream!', newmentionscount);
      ...
      case 'user': //user stream 
        ...
        if (this.cs.filter.items.length < this.cs.items.length) {
          i = 0;
                                                                                                    _D('F:parseitems', 'parsing from',this.cs.filter.items.length,'to',this.cs.items.length-1);
          filteredcount = this.cs.filter.items.length;
          nextid = this.cs.filter.items.length;
          itemcount = this.cs.items.length;
          while (filteredcount < itemcount) {
              ...
              $('span.tweet-actions', item)
                ...
                .before('<a class="tf-timestamp" href="/#!/'+tweet.username+'/status/'+tweet.tweetid+'" title="'+tweet.localtime.timestamp+'">'+tweet.localtime.prettytimestamp+'</a> '+
                        ...
                        '<span class="tf-via">via '+tweet.via+'</span>');
                                                                                                    _D(f, 'I:itemid', itemid, 'found, id:',id);
            } else {
                                                                                                    _D(f, 'W:itemid not found in filtered:',itemid);
              reparseitems = true;      
            }
            //item = null;  
          }
          if (this.getoption('scroll-lock') && this.cs._sinceId !== this.status.scrollsinceid && this.stream.newitemsloaded) {
                                                                                                    _D('F:lockscroll', 'polling event in parsestream, sinceid cs:',this.cs._sinceId,', sinceid stored:', this.status.scrollsinceid, ' - new items loaded:',this.stream.newitemsloaded);
  ...
  Tweetfilter.prototype.refreshuser = function() {
    ...
    //tests so far show that it can store up to 1.8 mil. relationships in approx. 4.9MB (5MB is local storage limit)
    //this.user.id= '';
    //this.user.name= '';
    //</debug>
  };
  ...
  Tweetfilter.prototype.debugfriends = function() { //call in firebug console: twtfilter.debugfriends()
    ...
    for (var i=0,item;i<localStorage.length && (item = localStorage.key(i));i++) {
      if (typeof item==='string' && item !== ':FRIENDS:' && item.indexOf(':FRIENDS:')===0) {
        var friends = localStorage.getItem(item);
        console.log('found packet', packet, ':', friends.substr(0, 100));
        try {
          console.log('packet', packet, 'is compressed');
          try {
            cs = friends.length; //compressed size
            totalsize+=cs;
            friends = twtfilter.decodelzw(friends);
            ucs = friends.length;
            friends = JSON.parse(friends);
            following = friends[0] ? friends[0].split(',') : '';
            follower = friends[1] ? friends[1].split(',') : ''; 
            mutual = friends[2] ? friends[2].split(',') : '';
            console.info('packet', packet, 'unpacked:', friends, ', contents:', follower.length, 'followers, ', following.length, 'follower, ',mutual.length, 'mutual friends, ratio:', Math.round(cs/ucs*100)/100, '(', cs, 'compressed,', ucs, 'uncompressed)');
            people+=follower.length+following.length+mutual.length;
          } catch(e) {
            console.warn('packet', packet, 'failed to unpack:', e);
          }
        } catch(e) {
          console.warn('packet', packet, 'is not valid json');
        }
        packet++;
      }
    }
    console.info('stored', people, 'friends using ~', Math.round(totalsize/1024,2)+' kB (', totalsize, 'Bytes)');
  };
      ...
      if (!this.timeids.refreshfriends || this.timeids.refreshfriends === -1) {
        this.timeids.refreshfriends = window.setInterval((function(e) {
          ...
          this.poll('refreshfriends');
                                                                                                    ...
                                                                                                    _D(f, 'W:canceling timed polling of checkmentions');
  ...
  Tweetfilter.prototype.fetchfriends = function() {
    ...
    if (!friends.fetching) {
      ...
      this.clearfriends();
                                                                                                    _D(f, 'W:Initializing fetch from API', friends);
      this.setvalue(':FRIENDS:', friends);
      this.friends.fids = {}; //reset fetched friend ids (friend status display is fed from uids)
                                                                                                    _D(f, 'first fetch ', friends.fetching);
    }
    if (friends.nextcursor !== '0' && friends.currentcursor !== friends.nextcursor) {
                                                                                                    _D(f, 'going on to next cursor ', friends.fetching, ':', friends.currentcursor,'=>', friends.nextcursor);
      ...
      if (friends.fetching === 'friendIds' && friends.currentcursor === '-1' && !this.friends.expires) {
        ...
        if (peoplecount > 42000 && !this.friends.expires && friends.fetching === 'friendIds' && friends.currentcursor==='-1' && friends.nextcursor==='0') { //only show on first load
          this.showmessage('Tweetfilter is loading friends, you can<br> use <b>show friend status</b> soon...', {timeout: 4200});
                                                                                                    ...
                                                                                                    _D(f, 'W:initializing next fetch ', friends.fetching, ' with cursor:', friends.currentcursor);
      currentuser[friends.fetching]({
        cursor: friends.nextcursor,
        success: (function(ids, cursor) {
                                                                                                    _D(f, 'evaluating:', friends.fetching, ', got', ids.length,'results, next cursor:', cursor.response.next_cursor_str);
          ...
          try {
            ...
            if (mutual.length) uids[2] = mutual.join(','); 
                                                                                                    _D(f, 'I:saving packet', this.friends.fetchedpackets, JSON.stringify(uids));
            this.setvalue(':FRIENDS:'+(this.friends.fetchedpackets++), this.pack(uids));
            
            if (friends.nextcursor !== '0') { //0 = no more to fetch
                                                                                                    _D(f, 'more ', friends.fetching, ' to fetch, next cursor:',friends.nextcursor);
              ...
              if (unids.length) {
                twttr.API.User.findAll(unids, {
                  stringifiedIds: true,
                  success: (function (result) {
                    result.each((function(user) {
                      unfollowers[user.idStr] = {
                        image: user.profileImageUrl,
                        name: user.screenName
                      };
                    }).bind(this));
                    this.setexvalue(this.user.name, unfollowers, 'unfollowers', false); //don't expire; clear manually   
                    this.poll('refreshactivities');
                  }).bind(this),
                  error: function () {
                                                                                                    _D(f, 'E:error fetching unfollowers', arguments);
                  }
                })
              }*/
              this.friends.uids = this.friends.fids;
              this.friends.fids = {};
                                                                                                    _D(f, 'finished fetching.', friends);
              this.friends.loading = false;
              this.setvalue(':FRIENDS:', friends);
              this.refreshoptions();
              this.poll('refreshfriendscss');
                                                                                                    ...
                                                                                                    _D(f, 'W:disabling friend status due to error',e);
            this.clearfriends();
  ...
  Tweetfilter.prototype.checktweet = function(tweet, search) {
     ...
     if (cs && this.stream.isready()) {
      ...
      for (var s=0, smax=searches.length; s < smax; s++) {
        query=searches[s];
        if (!query.active ||
           (query.retweets && !tweet.rt) ||
           (query.media && !tweet.ismedia) || 
           (query.links && !tweet.haslinks)) {
           continue;
        } 
        ismatch = false;
        ...
        if (query.user) {
          if (query.regular) {
            if (query.retweets) {
              ismatch = query.regex.test(tweet.rt.username);
            } else {
            ...
            if (query.retweets) {
              ismatch = tweet.rt.username === query.search;
            } else {
  ...
  Tweetfilter.prototype.addquery = function(query, enabled, stream) {
    ...
    var types = {
      user:    /^\-?(?:(rt|retweets|re|replies|me|media|li|links)\s+)?(?:f(?:rom)?\:\s*@?|@@)([A-Za-z0-9\_]{1,15}|(?:.+\=)?\/.+\/)$/, // from: @tweetfilterjs or from:tweetfilterjs or f:tweetfilterjs or @@tweetfilterjs (short syntax)
      source:  /^\-?(?:(rt|retweets|re|replies|me|media|li|links)\s+)?(?:v(?:ia)?\:\s*|\:@)(.+)$/, // via: echophon or via:web or v:iphone or :@sometwitterbot (short syntax)
      name:    /^\-?(?:(rt|retweets|re|replies|me|media|li|links)\s+)?(?:by?\:\s*|\?@)(.+)$/, // by: john doe or ?@john doe  (short syntax)
    ...
    var matches, regularmatch, exactmatch, tweettype;
    for (var type in types) {
      ...
      if (matches) {
        ...
        if (~['exact','simple'].indexOf(type)) {
          ...
          search.label = matches[2]; //preserve case for labels and regular expressions
          search.search = search.sortby = matches[2].toLowerCase(); //always do case insensitive search / sort
          if (matches[1]) {
            tweettype = matches[1];
            if (~['rt','retweets'].indexOf(tweettype)) {
              tweettype = 'retweets ';
            } else 
            if (~['re','replies'].indexOf(tweettype)) {
              tweettype = 'replies ';
            } else 
            if (~['me','media'].indexOf(tweettype)) {
              tweettype = 'media ';
            } else 
            if (~['li','links'].indexOf(tweettype)) {
              tweettype = 'links ';
            }
            search[tweettype.replace(/\s$/,'')] = true;
          } else tweettype = '';
        }
        search.type = type;
        ...
        switch(type) {
          ...
          case 'user': //match tweets by user. this filter is always "exact"
            search.user = true;
            search.index = (search.excluded ? '-' : '')+tweettype+'from:@'+search.search;
            search.raw = (search.excluded ? '-' : '')+tweettype+'from:@'+search.label;
          ...
          case 'source': //match tweets by source (via)
            search.source = true;
            search.index = (search.excluded ? '-' : '')+tweettype+'via:'+search.search;
            search.raw = (search.excluded ? '-' : '')+tweettype+'via:'+search.label;
          ...
          case 'name': //match tweets by (real?)name of user
            search.name = true;
            search.index = (search.excluded ? '-' : '')+tweettype+'by:'+search.search;
            search.raw = (search.excluded ? '-' : '')+tweettype+'by:'+search.label;
  ...
  Tweetfilter.prototype.checknewmentions = function() {
                                                                                                    var f=_F('checknewmentions');
    if (this.getoption('alert-mention') || this.getoption('alert-sound-mention')) {
                                                                                                    _D(f, 'I:Checking for new mentions');
      var storedmentionsinceid = this.getstoredvalue('mentionsinceid', -1);
      var mentionslastchecked = this.getstoredvalue('mentionslastchecked', -1);
      if (twttr.util.natcompare(storedmentionsinceid,this.status.mentionsinceid) > 0) {
        this.status.mentionsinceid = storedmentionsinceid;
      }
      if (mentionslastchecked > -1 && (new Date).getTime() - mentionslastchecked < 2 * 60 * 1000) {
                                                                                                    _D(f, 'W:Mentions are not expired, exiting', (new Date).getTime() - mentionslastchecked);
        this.status.mentionslastchecked = mentionslastchecked;
        this.savesettings();
        return true;
      }
      twttr.currentUser.mentions({
        ...
        success: (function(data, result) {
          this.status.mentionslastchecked = (new Date()).getTime();
          if (result.response.length) {
            if (this.status.mentionsinceid !== result.response[0].id) {
              if (this.status.mentionsinceid <= 0) { 
                ...
                this.savesettings();
                                                                                                    _D(f, 'W:first fetch, not alerting.', 'new sinceid:'+this.status.mentionsinceid);
                return;
              }
              if (twttr.util.natcompare(result.response[0].id, this.status.mentionsinceid) > 0) {
                ...
                if (newmentionscount > 0) {
                  ...
                  if (this.getoption('alert-sound-mention')) {
                                                                                                    _D('F:playsound','playing sound in checknewmentions!');
                    this.playsound();
                  }
    ...
    if (this.getoption('alert-message') || this.getoption('alert-sound-message')) {
      if (twttr.messageManager) { //message manager loaded
        if (!twttr.messageManager._tfbound) {
                                                                                                    _D(f, 'I:new internal message manager found, binding!');
          if (!twttr.messageManager._pollId) {
                                                                                                    _D(f, 'W:message manager is not polling yet, waiting!');
            return false;
          }
          if (twttr.messageManager._pollId) {
                                                                                                     _D(f, 'setting new polling interval to 1min!');
          ...
          twttr.messageManager.bind('messagesArrived', (function(e,messages) {
            ...
            if (twttr.util.natcompare(storedmessagesinceid, this.status.messagesinceid) > 0) {
            ...
            for (var m=0,mlen=messages.array.length,message;m<mlen && (message=messages.array[m]);m++) {
              if (''+message.senderId !== this.user.id && twttr.util.natcompare(message.id, this.status.messagesinceid) > 0) {
                                                                                                    _D(f, 'new message: ', message.id, 'from', message.senderScreenName);
                if (twttr.util.natcompare(newsinceid, message.id) > 0) {
                                                                                                    _D(f, 'set as new sinceid: ', message.id);
            ...
            if (newmessagescount) {
              this.status.messagesinceid = newsinceid;
                                                                                                    _D(f, 'set new sinceid:', newsinceid);
                                                                                                    _D(f, 'I:got',newmessagescount,'new messages, notifying!', message.id);
            ...
            this.status.messageslastchecked = (new Date()).getTime();
            this.savesettings();
          }).bind(this));
        ...
        if (this.timeids.checknewmessages && this.timeids.checknewmessages !== -1) { //triggered by event: cancel poll
                                                                                                    _D(f, 'W:canceling timed polling of checkmessages, event now bound to messagemanager');
        ...
        var storedmessagesinceid = this.getstoredvalue('messagesinceid', -1);
        var messageslastchecked = this.getstoredvalue('messageslastchecked', -1);
        if (twttr.util.natcompare(storedmessagesinceid,this.status.messagesinceid) > 0) {
          this.status.messagesinceid = storedmessagesinceid;
        }  
        if (messageslastchecked > -1 && (new Date).getTime() - messageslastchecked < 2 * 60 * 1000) {
                                                                                                      _D(f, 'W:Messages are not expired, exiting', (new Date).getTime() - messageslastchecked);
          this.status.messagesinceid = storedmessagesinceid;
          this.status.messageslastchecked = messageslastchecked;
          this.savesettings();
          return true;
            ...
            if (result.response.length) {
              if (this.status.messagesinceid === -1) {
                this.status.messagesinceid = result.response[0].id;
                                                                                                      _D(f, 'sinceid:'+this.status.messagesinceid);
                this.savesettings();
                return;
              }
        ...
        if (!this.timeids.checknewmessages || this.timeids.checknewmessages === -1) { //set up continous poll
                                                                                                    _D(f, 'I:enabled timed polling of checknewmessages');
          this.timeids.checknewmessages = window.setInterval((function() {
                                                                                                    _D(f, 'I:timed polling checkmessages!');
            this.poll('checknewmessages');
          }).bind(this), 60000);
        }
      }
      ...
      if (this.timeids.checknewmessages && this.timeids.checknewmessages !== -1) { //not active: cancel poll
                                                                                                    _D(f, 'W:canceling timed polling of checkmessages');
        window.clearInterval([this.timeids.checknewmessages, this.timeids.checknewmessages=-1][0]);
      }
    }
    return true; //always stop polling
  ...
  Tweetfilter.prototype.tweetclicklink = function(e) {
    ...
    var href = e.target.getAttribute('href'), 
        ...
        target = href;
                                                                                                    _D(f, 'found data on link:', linkdata);
  ...
  Tweetfilter.prototype.savedsearchclick = function(e) {
                                                                                                    ...
                                                                                                    _D(f, 'setting decider feature to ',+!this.getoption('search-realtime'));
    if (e.which === 1) {
      if (e.type === 'mousedown') {
        var target = e.currentTarget.getAttribute('href');
        if (!target) {
  ...
  Tweetfilter.prototype.tweetclickreply = function(e) {
    this._tweetreplyto = $(e.target).closest('.tweet-row').find('a.tweet-timestamp').attr('href');
                                                                                                    _D('F:tweetclickreply', 'replying to status id',  this._tweetreplyto);
  };
  ...
  Tweetfilter.prototype.tweebuttonclick = function(e) {
    ...
    var tweetbox = $(e.currentTarget).closest('.tweet-box'),
        ...
        tweetbutton = tweetbox.find('a.tweet-button:not(.twee-button)');
                                                                                                    _D(f, 'tweet text (',tweettext.length,'): ', tweettext, tweebutton, tweetbutton)
  ...
  Tweetfilter.prototype.tweetclickretweeted = function(e) {
                                                                                                    var f=_F('tweetclickretweeted')    
                                                                                                    _D(f,e);
    var row = $(e.currentTarget).closest('div.tweet-row');
    if (row.length) {
      var target = $('div.tf-retweeters', row);
      var caption = $('span.tf-show-rt', row);
      if (target.length && target.is(':visible')) {
        target.hide();
        caption.html(caption.data().showtitle);
      } else {
        caption.data('showtitle', caption.html());
        caption.html(_("Please wait\u2026"));
        target.remove();
        ...
        twttr.API.Status.find(itemid).retweeters({
          ...
          success: (function(r) {
            ...
            caption.html(caption.data().hidetitle);
          }).bind(this)
        });
      }
    }
      ...
      for (var l=0, lmax=links.length, link; l<lmax && (link=links.eq(l)); l++) {
        ...
        switch(linkclass) {
          ...
          case 'twitter-timeline-link':
            var linkaddress = link.attr('data-ultimate-url');
            if (!linkaddress) linkaddress = link.attr('data-expanded-url');
  ...
  Tweetfilter.prototype.urlsresolved = function(urls) {
    ...
    if (this.timeids.parselinks && this.timeids.parselinks !== -1) {
                                                                                                    _D(f,'clearing parselinks timeout.');
      window.clearTimeout([this.timeids.parselinks, this.timeids.parselinks=-1][0]);
      this.timeids.parselinks = -1;
    }
                                                                                                    _D(f,'setting parselinks timeout.');
  ...
  Tweetfilter.prototype.addresolvedurl = function(shortened, expanded) {
    ...
    if (this.isexpanded(shortened, expanded)) {
      if (!this.cs.resolvedurls[shortened] || (this.isexpanded(this.cs.resolvedurls[shortened], expanded))) {
                                                                                                    _D(f,'adding new expanded url:', shortened, '=>', expanded);
        this.cs.resolvedurls[shortened] = expanded;
      } else {
                                                                                                    _D(f,'W:not adding/overwriting expanded url:', shortened, '=>', expanded, ', keeping:', this.cs.resolvedurls[shortened]);
        
      }
    } else {
                                                                                                    _D(f,'W:not adding possibly wrong expanded:', shortened, '=>', expanded);
    }
  };
  ...
  Tweetfilter.prototype.parselinks = function() {
    ...
    var links = $('a.twitter-timeline-link', this.cs.$node) //links in timeline
                .add('a.twitter-timeline-link', $('#details-pane-outer')), //links in open dashboard pane
    ...
    for (var l=0,llen=links.length,link;l<llen && (link=links.eq(l));l++) {
      ...
      if (linkdata.resolvedurl != linkdata.longurl) { //has link been expanded since last run
        ...
        link.data('tf', linkdata); //.addClass('tf');
                                                                                                    _D(f, 'linkdata:', linkdata);
        if (linkdata.tweeplus && this.getoption('enable-tweeplus')) {
          try {
                                                                                                  _D(f, 'I:twee+ link found');
            var tweetcontainer = link.closest('div.tweet-content'),
                textcontainer = link.closest('div.tweet-text-large'), 
              ...
              if (longtext) {
                ...
                if (tweetcontainer.length && !tweetcontainer.data().tweeplus) {
                  tweetcontainer.data('tweeplus', longtext);
                }
                if (textcontainer.length) {
        ...
        //<debug>
        if (id > -1) {
                                                                                                    _D(f, 'I:added link to text:', this.cs.filter.items[id].text);
                                                                                                    _D(f, 'I:checking tweet:', this.cs.filter.items[id]);
        } else {
...
/*    if (this._page === 'Home') {
  ...
  Tweetfilter.prototype.listexvalues = function(andclear) {
    for (var i=0,index;i<localStorage.length && (index = localStorage.key(i));i++) {
      if (typeof index==='string' && index.substr(0, 4) === ':TF:') {
        var cachedvalue = this.getvalue(index);
        console.log(cachedvalue.value, 'expires in',((new Date()).getTime()-cachedvalue.expires) / 60 / 1000, 'minutes');
        if (andclear) {
  ...
  Tweetfilter.prototype.clearexpiredvalues = function(clearall) {
    var now = new Date();
    if (typeof clearall !== 'boolean') clearall = false;
    for (var i=0,index;i<localStorage.length && (index = localStorage.key(i));i++) {
      if (typeof index==='string' && index.substr(0, 4) === ':TF:') {
        var cached = this.getvalue(index);
        if (cached && cached.hasOwnProperty('expires') && (clearall || cached.expires < now.getTime())) {
          this.setvalue(index, null);
        }
      }
    }
  ...
  Tweetfilter.prototype.getexvalue = function(name, defaultvalue, section) { 
    ...
    var cached = this.getvalue(targetindex), now = new Date();
                                                                                                    _D(f, 'getting cached value', targetindex, ':', cached);
    //<debug>
    if (cached && cached.expires) {
      _D(f, 'value expires in', (cached.expires-now.getTime()) / 1000 / 60, 'minutes');
    }
    //</debug>
    if (typeof cached === 'object' && cached.value && cached.expires && cached.expires > now.getTime()) {
                                                                                                    _D(f, 'I:current value:',cached.value);
      return cached.value;
    } 
                                                                                                    _D(f, 'W:cached value', targetindex, ' not set or expired:', cached);
  ...
  Tweetfilter.prototype.setexvalue = function(name, value, section, lifetimeseconds) {
    ...
    if (typeof lifetimeseconds !== 'number') {
      if (lifetimeseconds !== false) {
        lifetimeseconds = 86400; //one day
      } else {
        lifetimeseconds = 86400 * 420; //a year or so
      }
    } 
                                                                                                    _D(f, 'setting cached value', targetindex, ':', value, 'expiring in', lifetimeseconds / 60, 'minutes');
    var cached = {
  ...
  Tweetfilter.prototype.refreshactivities = function() {
    ...
    if (cached) {
      ...
      twttr.API.User.find(targetusername, (function(user) {
        user.reload((function(user) {
          ...
          ).attr({'data-followerscount': twttr.util.formatNumber(user.followersCount, twttr.pageLocale), 
                  'data-followingcount': twttr.util.formatNumber(user.friendsCount, twttr.pageLocale)});
          var title = $('h2.dashboard-profile-title', activities),
              titlehtml = '<img width="24" class="profile-dashboard" alt="'+this.encodehtml(user.name)+'" src="'+user.profileImageUrl+'"> @'+user.screenName;
          if (user.url) {
            titlehtml = '<a href="'+user.url+'" title="'+this.encodehtml(user.url)+'" target="_blank">'+titlehtml+'</a>'
          }
          title.html(titlehtml);
          $('div.user-info', activities).remove();
          title.after('<div class="tf user-info"><span class="bio">'+twttr.util.linkify(user.description)+'</span></div>');
          this.setexvalue(user.screenName, activities.html(), 'activities', 60 * 5);
        }).bind(this));      
      }).bind(this));    
    }
    if (following.length) {
      cached = this.getexvalue(targetusername, false, 'following');
      if (cached) {
        following.html(cached);
      } else {
        twttr.API.User.find(targetusername).friends({
          count: 12,
          success: (function(collection, data) {
            var userthumbs = [
               '<h2>',
                 '<a class="title-link" href="/#!/'+targetusername+'/following">',
                   '<span class="dashboard-component-title">'+_('Following')+'</span>',
                   '<span class="user-stat-link">'+$('.tf-activities > ul', activities).attr('data-followingcount')+'</span>',
                 '</a>',
               '</h2>',            
              '<ul class="inline-list">',
            ];
            if (data.response.users) {
              collection = data.response.users;
            } else return;
            for (var u=0,umax=collection.length,user;u<umax && (user=collection[u]);u++) {
              userthumbs.push([
                '<li class="user-thumb-list-member">',
                  '<a data-user-id="'+user.id_str+'" class="user-profile-link" href="/#!/'+user.screen_name+'">',
                    '<img width="24" height="24" src="'+user.profile_image_url+'" original-title="'+user.screen_name+'">',
                  '</a>',
                '</li>'].join("\n")              
              );
            }
            userthumbs.push('</ul>')
            following.html(userthumbs.join("\n"));
            this.setexvalue(targetusername, following.html(), 'following', 60 * 15);
          }).bind(this)
        });
      }
    }
    if (followers.length) {
      cached = this.getexvalue(targetusername, false, 'followers');
      if (cached) {
        followers.html(cached);
      } else {
        twttr.API.User.find(targetusername).followers({
          count: 12, 
          success: (function(collection, data) {
            var userthumbs = [
               '<h2>',
                 '<a class="title-link" href="/#!/'+targetusername+'/followers">',
                   '<span class="dashboard-component-title">'+_('Followers')+'</span>',
                   '<span class="user-stat-link">'+$('.tf-activities > ul', activities).attr('data-followerscount')+'</span>',
                 '</a>',
               '</h2>',            
              '<ul class="inline-list">',
            ];
            ...
            userthumbs.push('</ul>')
            followers.html(userthumbs.join("\n"));
            this.setexvalue(targetusername, followers.html(), 'followers', 60 * 15);
          }).bind(this)
        });
      }      
    }
    ...
    if (unfollowers.length) {
      if (this.getoption('show-unfollowers')) {
        ...
        if (!this.isemptyobj(cached)) {
          var userthumbs = [
             '<h2>',
               '<a class="title-link" href="/#!/'+targetusername+'/followers">',
                 '<span class="dashboard-component-title">'+_('Unfollowers')+'</span>',
                 '<span class="user-stat-link">'+twttr.util.objKeys(cached).length+'</span>',
               '</a>',
               '<span class="dashboard-component-subtitle">',
                  '<span class="dot">·</span>',
                  '<a class="clear-unfollowers" href="#">clear all</a>',
                '</span>',
             '</h2>',            
          ...
          for (var uid in cached) {
            ...
            userthumbs.push([
              '<li class="user-thumb-list-member">',
                '<a data-user-id="'+uid+'" class="user-profile-link" href="/#!/'+user.name+'">',
                  '<img width="24" height="24" src="'+user.image+'" original-title="'+user.name+'">',
                '</a>',
              '</li>'].join("\n")              
            );
          }
          userthumbs.push('</ul>')
          unfollowers.html(userthumbs.join("\n"));
          unfollowers.css({'display':'block'});      
        } else {
  ...
  Tweetfilter.prototype.clearunfollowers = function() {
    ...
    if (unfollowers.length) {
      unfollowers.empty().css({'visibility':'hidden'});
  ...
  */
  ...
  Tweetfilter.prototype.widgethtml = function() {
    return [
      ...
      //      '<a href="#" class="button advanced" title="Advanced Filter">Advanced</a>',
        ...
        '<div id="tf-customize" data-tab="customize" class="tf-tab-customize">',
          '<ul class="tf-tabs tf-tabs-customize">',
            '<li class="tf-tab tf-tab-tweetfilter active tweetstream">',
            ...
            '</li>',
            this.beta ? ['<li class="tf-tab tweetstream">',
              '<a data-tab="beta">Beta</a>',
            '</li>'].join("\n") : '',
          '</ul>',
          '<div data-tab="tweetfilter" class="active tweetstream activitystream">',
            '<ul class="checks">',
...
//              '<li class="disabled"><a data-option="expand-activity"><b></b>expand activity</a></li>',
    ...
    if (!this.widget) {
      ...
      .delegate('a.love', 'click', (function() {
        new twttr.widget.TweetDialog({
          modal: false,
          draggable: true,
          defaultContent: "Tweetfilter \u2665 browser extension for #newtwitter: http://tweetfilter.org (via @tweetfilterjs)",
          template: {
            title: _("Thank you! :)")
          },
          origin: "Tweetfilter "+this.version
  ...
  Tweetfilter.prototype.refreshfilterlist = function() {
                                                                                                    var f=_F('refreshfilterlist');
    if (!this.widget) {
                                                                                                    _D(f,'W:widget not ready. repolling:', this.stream.isusers() || this.stream.istweets())
    ...
    for (var i=0,len=this.queries.length; i<len; i++) {
      ...
      if (query.retweets) {
        action = action + ' retweets';
      } else 
      if (query.replies) {
        action = action + ' replies';
      } else 
      if (query.media) {
        action = action + ' media';
      } else 
  ...
  Tweetfilter.prototype.refreshfiltercss = function() {
    ...
    if (this.getoption('filter-disabled') || !this.stream.isready() || !this.stream.istweets()) {
      style.push('.tf-actions > a.tf.menu { display:none !important; }'); //hide the "add to filter" dropdown in stream while disabled
                                                                                                    _D(f, 'W:suspending filter. stream itemtype: ', this._stream.itemtype, ', mode:', this.stream.mode, ', filter disabled:', this.getoption('filter-disabled'));
    ...
    if (this.getoption('tweets-fill-page') && tweetsvisible < 10) {
                                                                                                    _D(f,'W:fetching more items (until 10 reached), currently visible:', tweetsvisible);
      this.cs.getMoreOldItems();
    }
    return true;
  };
  ...
  Tweetfilter.prototype.refreshfriendscss = function() {
                                                                                                    var f=_F('refreshfriendscss');
    if (!this.stream.isready()) {
                                                                                                    _D(f, 'W:stream is not ready!')
      return false;
    }
    if (!this.getoption('show-friends')) {
                                                                                                    _D(f, 'W:option disabled by stream or user, or friends currently loading!')
      this.setcss('friends', '');
      return true;
    }
    if (this.friends.expires) { //is 0 at init, > 0 means it's loaded. we don't care here if it's expired, that's done in interval and refreshfriends
      ...
      this.setcss('friends', css.join("\n")); //friend status
                                                                                                    ...
                                                                                                    _D(f, 'W:friends have not been loaded!')
    } 
    return true;
  };
  ...
  Tweetfilter.prototype.refreshlayoutcss = function() { 
                                                                                                    var f=_F('refreshlayoutcss');
                                                                                                    _D(f, 'refreshing widget layout css');
  ...
  Tweetfilter.prototype.pack = function(data) {
    ...
    if (typeof data !== 'object') {
                                                                                                    _D(f,'invalid data', data);
      return false;
    }
    data = JSON.stringify(data);
                                                                                                    _D('packing', data.length, 'bytes');
  ...
  Tweetfilter.prototype.unpack = function(packed) {
                                                                                                    var f=_F('unpack');
    try {
                                                                                                    _D('unpacking', packed.length, 'bytes');
      return JSON.parse(this.decodelzw(packed));
    } catch (e) {
                                                                                                    _D(f, 'W:error unpacking', e);
    }
    return false;
  }; 
...
// <debug>
  var TweetfilterDebug = function() {
    this._debug = true; //turn on debug. use firefox with firebug. will be _very_ verbous with standard settings. will probably slow down the script.
    ...
    this._debugfunctions = ['_poll', 'parseitems', 'parsestream'];//which functions to debug (whitelist). empty array = debug all functions
    ...
    this._debuggroup = false;
    this._debugfuncs = true;
  };
  
 //debug log function. only called from outside the object scope or by single recursion
  TweetfilterDebug.prototype._log = function() {
    var args = Array.prototype.slice.call(arguments); //convert to array
    var that = window.tfdebug; //"this" is window.console
  ...
  window.tfdebug = new TweetfilterDebug;
  if (!!window.tfdebug._debug && !!window.console) {
    _D = function() {
      if (typeof console.log.apply === 'function') { //check for bug in firebug 1.8a for firefox 4
        window.tfdebug._log.apply(console, arguments);
...
{ 
  if (window.location.toString().match(/^https?\:\/\/twitter\.com\/(\?.*)?(#.*)?$/)) { //only run on twitter.com
    ...
    tfscript.id = 'tfs'+(~~(Math.random()*100000));  //random id to prevent fancy browser caching (?)
    tfscript.className = 'twtfilterscript';  //classname to identify script block
    var tfscripttext = TweetfilterScript.toString(); //assign the whole prototype code to script
